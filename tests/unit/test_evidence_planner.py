...
from src.core.data_models import (
    CaseData, Party, ContractInfo, Claim, ClaimList, CaseType, BreachInfo
)
...
def sample_case_data():
    ...
    return CaseData(
        ...
        breach=BreachInfo(
            ...
            breach_description="被告自2023年6月起未支付租金"
        )
    )
...
class TestEvidencePlanner:
    ...
    def test_plan_basic(self, sample_case_data, sample_claim_list):
        """测试基本规划功能"""
    ...
    def test_plan_includes_contract_evidence(self, sample_case_data, sample_claim_list):
        ...
        has_contract = any(
            r.type.value == "合同类" for r in result.requirements
        )
        assert has_contract
    ...
    def test_plan_includes_voucher_evidence(self, sample_case_data, sample_claim_list):
        ...
        has_voucher = any(
            r.type.value == "凭证类" for r in result.requirements
        )
        assert has_voucher
    ...
    def test_plan_with_llm_client(self, sample_case_data, sample_claim_list):
        """测试使用LLM客户端规划"""
        class MockLLMClient:
            def complete(self, prompt):
                return '{"requirements": []}'

        planner = EvidencePlanner(llm_client=MockLLMClient())
        result = planner.plan(sample_case_data, sample_claim_list)
...
class TestEvidencePlannerRequirements:
    ...
    def test_requirement_has_attachment_plan(self, sample_case_data, sample_claim_list):
        """测试证据需求包含附件规划"""
        planner = EvidencePlanner()
        result = planner.plan(sample_case_data, sample_claim_list)

        has_attachment = any(
            r.attachment is not None for r in result.requirements
        )
