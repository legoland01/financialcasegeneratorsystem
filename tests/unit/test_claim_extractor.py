...
class TestClaimExtractor:
    ...
    def test_extract_principal_amount(self):
        """测试提取本金金额"""
        text = "请求判令被告支付欠款本金人民币1,500,000元"
        extractor = ClaimExtractor()
        result = extractor.extract_from_text(text)

        principal = next((c for c in result.claims if c.type == "本金"), None)
        assert principal is not None
        assert principal.amount == 1500000.0
...
诉讼费用人民币5,000元由被告承担。
        ...
        assert result.litigation_cost == 5000.0
...
class TestClaimExtractorRegex:
    ...
    def test_amount_with_rmb_prefix(self):
        """测试带人民币前缀"""
        text = "请求判令被告支付欠款本金人民币1,000,000元"
        extractor = ClaimExtractor()
        result = extractor.extract_from_text(text)
