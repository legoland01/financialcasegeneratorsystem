...
### 13.1 黑盒测试发现问题（2026-01-31）

| 问题ID | 问题描述 | 严重程度 | 解决方案 |
|--------|---------|---------|---------|
| QB-001 | 执行证书存在大量占位符（X年月日、人民币X元） | 严重 | 后处理器自动填充 |
| QB-002 | 融资租赁合同缺少当事人信息 | 严重 | 完善Prompt上下文 |
| QB-003 | 租金支付记录金额格式错误 | 中等 | 后处理器格式化 |
| QB-004 | 证据ID重复（多个E003、E004） | 中等 | 重新设计ID生成器 |
| QB-005 | PDF存在脱敏标记 | 严重 | 后处理+二次验证 |
