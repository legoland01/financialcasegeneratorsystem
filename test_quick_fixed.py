...
"""
    ...
    try:
        from src.utils import LLMClient, load_prompt_template, save_json, parse_llm_json_response
...
"""
        ...
        if result:
            ...
            logger.warning("JSON解析失败，但已保存原始响应")
            
            # 保存原始响应
            output_path = Path("outputs/test_response.txt")
            output_path.parent.mkdir(parents=True, exist_ok=True)
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(response)
            
            logger.success(f"原始响应已保存到: {output_path}")
            return False
