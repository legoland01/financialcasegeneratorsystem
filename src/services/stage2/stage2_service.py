...
class Stage2Service:
    ...
    def generate_answer(
        ...
        # 加载提示词
        prompt_path = self.prompt_dir / "stage2" / "2.1_答辩状生成.md"
...
被告证据归属规划：
{json.dumps(defendant_evidence, ensure_ascii=False, indent=2)}
...
"""
        ...
        # 保存结果
        output_path = self.output_dir / "stage2" / "民事答辩状.txt"
        output_path.parent.mkdir(parents=True, exist_ok=True)
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(clean_response)
        
        # 保存质量报告
        report_path = self.output_dir / "stage2" / "民事答辩状_质量报告.json"
        save_json(quality_report, str(report_path))
        
        logger.info(f"民事答辩状已生成并保存到 {output_path}")
    ...
    def generate_evidence_package(
        ...
        Returns:
            生成的证据包和质量检查报告
        """
        logger.info(f"开始生成被告证据包(证据组{evidence_group_index})")
        ...
        # 加载提示词
        prompt_path = self.prompt_dir / "stage2" / "2.2_证据包生成.md"
        prompt = load_prompt_template(str(prompt_path))
...
"""
        ...
        # 保存结果
        output_path = self.output_dir / "stage2" / f"被告证据包_证据组{evidence_group_index}.txt"
        output_path.parent.mkdir(parents=True, exist_ok=True)
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(clean_response)

        # 保存质量报告
        report_path = self.output_dir / "stage2" / f"被告证据包_证据组{evidence_group_index}_质量报告.json"
        save_json(quality_report, str(report_path))

        logger.info(f"被告证据包(证据组{evidence_group_index})已生成并保存到 {output_path}")
    ...
    def generate_evidence_files(
        ...
        logger.info("开始使用新架构生成被告证据文件")

        evidence_planning = stage0_data["0.5_证据归属规划"]
        evidence_output_dir = self.output_dir / "stage2" / "evidence"
        ...
        logger.info(f"被告证据文件生成完成，共 {evidence_index.get('证据总数', 0)} 个证据")
    ...
    def generate_procedural_files(
        ...
        # 加载提示词
        prompt_path = self.prompt_dir / "stage2" / "2.3_程序文件生成.md"
        prompt = load_prompt_template(str(prompt_path))
...
"""
        ...
        # 保存结果
        output_path = self.output_dir / "stage2" / "被告程序性文件.txt"
        output_path.parent.mkdir(parents=True, exist_ok=True)
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(clean_response)
        
        # 保存质量报告
        report_path = self.output_dir / "stage2" / "被告程序性文件_质量报告.json"
        save_json(quality_report, str(report_path))
        
        logger.info(f"被告程序性文件已生成并保存到 {output_path}")
    ...
    def run_all(self, stage0_data: Dict[str, Any], use_new_architecture: bool = True) -> Dict[str, Any]:
        ...
        # 生成民事答辩状
        answer = self.generate_answer(stage0_data)

        # 生成证据包
        logger.info("开始生成被告证据包")
        ...
        if use_new_architecture:
            ...
            if evidence_groups:
                logger.info(f"发现 {len(evidence_groups)} 个被告证据组")
        ...
        # 保存完整结果
        output_path = self.output_dir / "stage2" / "defendant_package.json"
        save_json(result, str(output_path))
        logger.info(f"阶段2完成，完整结果已保存到 {output_path}")
        if use_new_architecture:
            evidence_count = evidence_package_result.get("证据总数", 0)
            logger.info(f"使用新架构，共生成 {evidence_count} 个独立证据文件")
        elif evidence_package_result:
            logger.info(f"共生成 {len(evidence_package_result)} 个被告证据组")
